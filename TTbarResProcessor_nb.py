...
class TTbarResProcessor(processor.ProcessorABC):
    def __init__(self, prng=RandomState(1234567890), htCut=950., minMSD=105., maxMSD=210.,
                 tau32Cut=0.65, ak8PtMin=400., bdisc=0.8484, #btag_sf=BTagScaleFactor("DeepCSV_106XUL17SF_V2.csv", "tight"),
...
#********************************************************************************************************************#
            ...
            'b_eff_numerator_s01': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            'b_eff_numerator_s02': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            'b_eff_numerator_s11': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            'b_eff_numerator_s12': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            
            'b_eff_denominator_s01': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            'b_eff_denominator_s02': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            ...
            'c_eff_numerator_s11': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            'c_eff_numerator_s12': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            
            'c_eff_denominator_s01': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            'c_eff_denominator_s02': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            ...
            'udsg_eff_numerator_s11': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            'udsg_eff_numerator_s12': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
            
            'udsg_eff_denominator_s01': hist.Hist("Counts", dataset_axis, cats_axis, subjetpt_axis, subjeteta_axis),
...
#********************************************************************************************************************#
    ...
    def BtagUpdater(self, subjet, b_eff, ScaleFactorFilename, FittingPoint, OperatingPoint):  
        """
...
#         # --- test to see what scale factors look like --- #
#         btag_sf = BTagScaleFactor("DeepCSV_106XUL17SF_V2.csv", "tight")
#         BSF = btag_sf.eval("central", SubJet01.hadronFlavour, abs(SubJet01.eta), SubJet01.pt, ignore_missing=True)
#         print(BSF)
#         print('--------------------------------------------------------')
        ...
        if self.CalcEff_MC == True: # Get 'flavor' tagging efficiency from MC
            if 'JetHT' not in dataset:
                ...
                flav_s01 = np.abs(ak.flatten(SubJet01.hadronFlavour))
...
#                 Eff_udsg_Denom_eta_s12 = ak.flatten(Eff_udsg_Denom_eta_s12)
        ...
        if self.ApplySF == True: # Apply b Tag Scale Factors, BSF
            if 'JetHT' not in dataset:
                if self.UseEfficiencies == False: # Define weights solely from BSF to weight each btag category region
                    ...
                    #btag_weights = np.array([ ['w00', 'w01', 'w02'], [None, 'w11', 'w12'], [None, None, 'w22'] ]) 
                    ...
                    # ---- w(0|0) ---- #
                    #btag_weights[0][0] = np.ones_like(BSF_s0)
                    btag_wgts['0b'][0] = np.where(btag0, np.ones_like(BSF_s0), 0.)
                    
                    # ---- w(0|1) and w(1|1) ---- #
                    #btag_weights[0][1] = np.where(btag1, np.where(btag_s0, 1.-BSF_s0, 1.-BSF_s1), 1.)
                    #btag_weights[1][1] = np.where(btag1, np.where(btag_s0, BSF_s0, BSF_s1), 1.) 
                    ...
                    # ---- w(0|2), w(1|2), w(2|2) ---- #
                    #btag_weights[0][2] = np.where(btag2, (1 - BSF_s0)*(1 - BSF_s1), 1.) 
                    #btag_weights[1][2] = np.where(btag2, (1 - BSF_s0)*BSF_s1 + BSF_s0*(1 - BSF_s1), 1.) 
                    #btag_weights[2][2] = np.where(btag2, BSF_s0*BSF_s1, 1.) 
...
#                     print()
        ...
        # ---- Delta R Test between a Subjet and its neighboring quark ---- #
        # ---- Check if GenPart's id matches with subjet's given hadron flavor ---- #
        ...
        isQuarkWithinRadiusb01 = deltaR_b01[np.arange(ak.size(deltaR_b01,0)),ak.to_numpy(minimumR_indexb01)] < 0.4 
        ...
        deltaR_b01_min = deltaR_b01[np.arange(ak.size(deltaR_b01,0)),ak.to_numpy(minimumR_indexb01)]
        ...
        isQuarkWithinRadiusb02 = deltaR_b02[np.arange(ak.size(deltaR_b02,0)),ak.to_numpy(minimumR_indexb02)] < 0.4 
        ...
        deltaR_b02_min = deltaR_b01[np.arange(ak.size(deltaR_b02,0)),ak.to_numpy(minimumR_indexb02)]
        ...
        isQuarkWithinRadiusb11 = deltaR_b11[np.arange(ak.size(deltaR_b11,0)),ak.to_numpy(minimumR_indexb11)] < 0.4 
        ...
        deltaR_b11_min = deltaR_b01[np.arange(ak.size(deltaR_b11,0)),ak.to_numpy(minimumR_indexb11)]
        ...
        isQuarkWithinRadiusb12 = deltaR_b12[np.arange(ak.size(deltaR_b12,0)),ak.to_numpy(minimumR_indexb12)] < 0.4 
        ...
        deltaR_b12_min = deltaR_b12[np.arange(ak.size(deltaR_b12,0)),ak.to_numpy(minimumR_indexb12)]
        ...
        #print('bquark contamination = ', bquark_contamination01)
        pairing_c01 = pairing_c01[keepEvents_c01]#, pairing_c01 = pairing_c01[bquark_contamination01]
        print('pairing pt = ', pairing_c01.slot0.pt)
        deltaR_c01 = pairing_c01.slot0.delta_r(pairing_c01.slot1) 
        minimumR_indexc01 = ak.argmin(deltaR_c01, axis=-1) 
        isQuarkWithinRadiusc01 = deltaR_c01[np.arange(ak.size(deltaR_c01,0)),ak.to_numpy(minimumR_indexc01)] < 0.4 
        ...
        deltaR_c01_min = deltaR_c01[np.arange(ak.size(deltaR_c01,0)),ak.to_numpy(minimumR_indexc01)]
        ...
        isQuarkWithinRadiusc02 = deltaR_c02[np.arange(ak.size(deltaR_c02,0)),ak.to_numpy(minimumR_indexc02)] < 0.4 
        ...
        deltaR_c02_min = deltaR_c02[np.arange(ak.size(deltaR_c02,0)),ak.to_numpy(minimumR_indexc02)]
        ...
        isQuarkWithinRadiusc11 = deltaR_c11[np.arange(ak.size(deltaR_c11,0)),ak.to_numpy(minimumR_indexc11)] < 0.4 
        ...
        deltaR_c11_min = deltaR_c11[np.arange(ak.size(deltaR_c11,0)),ak.to_numpy(minimumR_indexc11)]
        ...
        isQuarkWithinRadiusc12 = deltaR_c12[np.arange(ak.size(deltaR_c12,0)),ak.to_numpy(minimumR_indexc12)] < 0.4 
        ...
        deltaR_c12_min = deltaR_c12[np.arange(ak.size(deltaR_c12,0)),ak.to_numpy(minimumR_indexc12)]
...
#         isGenPart_lightquark = (np.abs(GenParts.pdgId) == 1 | np.abs(GenParts.pdgId) == 2 | 
#                                 np.abs(GenParts.pdgId) == 3 | np.abs(GenParts.pdgId) == 21)
        ...
        isQuarkWithinRadiusl01 = deltaR_l01[np.arange(ak.size(deltaR_l01,0)),ak.to_numpy(minimumR_indexl01)] < 0.4 
        ...
        deltaR_l01_min = deltaR_l01[np.arange(ak.size(deltaR_l01,0)),ak.to_numpy(minimumR_indexl01)]
        ...
        isQuarkWithinRadiusl02 = deltaR_l02[np.arange(ak.size(deltaR_l02,0)),ak.to_numpy(minimumR_indexl02)] < 0.4 
        ...
        deltaR_l02_min = deltaR_l02[np.arange(ak.size(deltaR_l02,0)),ak.to_numpy(minimumR_indexl02)]
        ...
        isQuarkWithinRadiusl11 = deltaR_l11[np.arange(ak.size(deltaR_l11,0)),ak.to_numpy(minimumR_indexl11)] < 0.4 
        ...
        deltaR_l11_min = deltaR_l11[np.arange(ak.size(deltaR_l11,0)),ak.to_numpy(minimumR_indexl11)]
        ...
        isQuarkWithinRadiusl12 = deltaR_l12[np.arange(ak.size(deltaR_l12,0)),ak.to_numpy(minimumR_indexl12)] < 0.4 
        ...
        deltaR_l12_min = deltaR_l12[np.arange(ak.size(deltaR_l12,0)),ak.to_numpy(minimumR_indexl12)]
        deltaR_l12_lessthanAK4 = deltaR_l12_new[isQuarkWithinRadiusl12]
        
        
#         print('ALL SHAPE SIZES ARE FLATTENED')
#         print()
#         print('-----------------------------------------------------------------------------')
#         print('SubJet01 pt               = ', SubJet01.p4.pt)
#         print('SubJet01 pt shape               = ', np.shape( ak.to_numpy(ak.flatten(SubJet01.p4.pt)) ))
#         print()
#         print('Weights                        = ', evtweights[isQuarkWithinRadiusb01])
#         print('Weights shape                   = ', np.shape( ak.to_numpy(evtweights[isQuarkWithinRadiusb01]) ))
#         print()
#         print('-----------------------------------------------------------------------------')
#         print('pairing_01 pt                  = ', pairing_b01)
#         print('pairing_01 shape                = ', np.shape( ak.to_numpy(ak.flatten(pairing_b01)) ))
#         print()
#         print('deltaR_b01_lessthanAK4                     = ', deltaR_b01_lessthanAK4)
#         print('deltaR_b01_lessthanAK4 shape                = ', np.shape( ak.to_numpy(deltaR_b01_lessthanAK4) ))
#         print()
#         print('minimumR_index01               = ', minimumR_index01) # should be flat already
#         print('minimumR_index01 shape          = ', np.shape( ak.to_numpy(minimumR_index01) ))
#         print()
#         print('Should Subjet01 have b flavor? = ', isQuarkWithinRadius01)
#         print('SSbf shape                      = ', np.shape( ak.to_numpy(isQuarkWithinRadius01) ))
#         print()
#         print('pairs_of_interest, subjet pt   = ', pairing_01.slot0.pt)
#         print('pairs_of_interest shape         = ', np.shape( ak.to_numpy(ak.flatten(pairing_01.slot0.pt)) ))
#         print()
#         print('-----------------------------------------------------------------------------')
#         print('SubJet01 with b flavor pt      = ', SubJet01[isSubJet_bflavor].p4.pt)
#         print('SubJet01 with b flavor shape    = ', np.shape( ak.to_numpy(ak.flatten(SubJet01[isSubJet_bflavor].p4.pt)) ))
#         print()
#         print('Corresponding Weights          = ', evtweights[isSubJet_bflavor])
#         print('Weights shape                   = ', np.shape( ak.to_numpy(evtweights[isSubJet_bflavor]) ))
#         print()
        
#         print('SubJet01 with b quarks pt      = ', SubJet01_with_bquark.pt)
#         print('SubJet01 with b quarks shape    = ', np.shape( ak.to_numpy(SubJet01_with_bquark.pt) ))
#         print()
#         print('Corresponding Weights          = ', evtweights[isQuarkWithinRadius01])
#         print('Weights shape                   = ', np.shape( ak.to_numpy(evtweights[isQuarkWithinRadius01]) ))
#         print()
...
#         print('#################################################################################################')
        ...
        output['subjet01_cquark_distance'].fill(dataset = dataset,
                                               ...
                                               weight = ak.to_numpy(evtweights[isQuarkWithinRadiusbc1]))
...
#         p = np.absolute( ttbarcands.slot1.p4.p )
        ...
        df = pd.DataFrame({"momentum":p}) # Used for finding mistag values in LookUp Tables
        ...
        for ilabel,icat in labels_and_categories.items():
            ...
            if self.UseEfficiencies == False:
                
#                 if '0b' in ilabel:
#                     Weights = Weights*ak.flatten(btag_wgts['0b'][0] + btag_wgts['1b'][0] + btag_wgts['2b'][0])
#                 elif '1b' in ilabel:
#                     Weights = Weights*ak.flatten(btag_wgts['1b'][1] + btag_wgts['2b'][1]) 
#                 else:
#                     Weights = Weights*ak.flatten(btag_wgts['2b'][2]) 
...
# ************************************************************************************************************ #            
            ...
            output['b_eff_numerator_s01'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_b_Num_pT_s01[icat]),
                                              subjeteta = ak.to_numpy(Eff_b_Num_eta_s01[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['b_eff_numerator_s02'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_b_Num_pT_s02[icat]),
                                              subjeteta = ak.to_numpy(Eff_b_Num_eta_s02[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['b_eff_numerator_s11'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_b_Num_pT_s11[icat]),
                                              subjeteta = ak.to_numpy(Eff_b_Num_eta_s11[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['b_eff_numerator_s12'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_b_Num_pT_s12[icat]),
                                              subjeteta = ak.to_numpy(Eff_b_Num_eta_s12[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            ...
            output['c_eff_numerator_s01'].fill(dataset = dataset, anacat = ilabel,
                                              ...
                                              subjeteta = ak.to_numpy(Eff_c_Num_eta_s01[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['c_eff_numerator_s02'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_c_Num_pT_s02[icat]),
                                              subjeteta = ak.to_numpy(Eff_c_Num_eta_s02[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['c_eff_numerator_s11'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_c_Num_pT_s11[icat]),
                                              subjeteta = ak.to_numpy(Eff_c_Num_eta_s11[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['c_eff_numerator_s12'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_c_Num_pT_s12[icat]),
                                              subjeteta = ak.to_numpy(Eff_c_Num_eta_s12[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            ...
            output['udsg_eff_numerator_s01'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_udsg_Num_pT_s01[icat]),
                                              subjeteta = ak.to_numpy(Eff_udsg_Num_eta_s01[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['udsg_eff_numerator_s02'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_udsg_Num_pT_s02[icat]),
                                              subjeteta = ak.to_numpy(Eff_udsg_Num_eta_s02[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['udsg_eff_numerator_s11'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_udsg_Num_pT_s11[icat]),
                                              subjeteta = ak.to_numpy(Eff_udsg_Num_eta_s11[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
            output['udsg_eff_numerator_s12'].fill(dataset = dataset, anacat = ilabel,
                                              subjetpt = ak.to_numpy(Eff_udsg_Num_pT_s12[icat]),
                                              subjeteta = ak.to_numpy(Eff_udsg_Num_eta_s12[icat]),
                                              weight = ak.to_numpy(Weights[icat]))
